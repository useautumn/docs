...
You can use Autumn's React hooks and components to replace frontend/backend glue code and setup billing even faster.

<Info>
  React hooks and components are supported for apps with a frontend and backend in Typescript. Please use our [Server-side SDK](/getting-started/sdk) for other frameworks and languages.
</Info>
...
Server-side, mount the Autumn handler. This will create endpoints in the `/api/autumn/*` path, which will be called by Autumn's frontend React [hooks](/api-reference/hooks/useCustomer). These endpoints in turn call Autumn's API.
...
Call `checkout` to redirect the customer to a Stripe checkout page when they want to purchase the Pro plan. Once they've paid, Autumn will grant them access to the "messages" defined in Step 1.

If their payment details are already on file, no Checkout URL is required. [CheckoutDialog](/setup/shadcn#checkout-dialog) will open instead to let your customer confirm their plan switch.
...
<Tip>
  Alternatively, you can use the [PricingTable](/setup/shadcn#pricing-table)
  component, or [roll your own](/setup/roll) plan switching flows with our SDK.
</Tip>
