...
<Steps>
    <Step
        title="Connecting RevenueCat to Autumn"
    >
    ...
    <Step
        title="Mapping your RevenueCat products to Autumn products"
    >
        <Warning>
            Autumn currently does not support mapping RevenueCat products to Autumn products with the following attributes:
            - Prepaid items
            - Usage-based pricing

            We suggest using flat fee add-ons instead.
        </Warning>
    ...
    <Step
        title="Setting up the webhook integration"
    >
    ...
    <Step
        title="Setting up your code"
    >
        Now you can start using Autumn with RevenueCat. There is only one difference between using Autumn with RevenueCat and using Autumn with Stripe,
        you cannot call `attach()` or `checkout()` with RevenueCat. Instead you should either use RevenueCat's SDK or RevenueCat's paywalls. Both will
        automatically sync into Autumn. However once you have purchased a product or an add-on, you may then continue using `check()` or `track()` to manage your features.
        Any existing Autumn users can still access their balances and features as normal via the `check()` and `track()` functions.
        
        <Warning>
            You should internally handle blocking existing Autumn users from initiating subscriptions with RevenueCat. Autumn will try its best to block this if it detects it,
            however you should still gate this in your UI.
        </Warning>

        When initiating your `Purchases` instance, you must use the `appUserID` parameter to pass in the customer ID from your auth system.
        This will allow you to sync the customer ID into Autumn, whether its a new user or an existing user from Stripe.
        Additionally if you are using [Expo](https://expo.dev/), you can also use our [React hooks](/api-reference/hooks/useCustomer) too!
